...
const mapWithoutReturnList = ( list, fn, and ) => {
...
const removingClassesFrom = ( body ) => ( thisClass ) => 
  body.classList.remove( thisClass )

const stopThisEvent = ( evt ) => {
  evt.preventDefault()
  evt.stopPropagation()
}

const getThemeClassFrom = ( classList ) => 
  classList[ THEME ] + `-theme`

const andAddClassTheme = ( themeName ) => ({
    To: ( BODY ) => BODY.classList.add( themeName )
  })
...
const addThemes = ( evt ) => { 
  ...
  mapWithoutReturnList( 
    BODY.classList, 
    removingClassesFrom( BODY ), 
    andAddClassTheme
  )
  .and( getThemeClassFrom( evt.srcElement.classList ) )
  .To( BODY )
}

const addingClickToThem = ( theme ) => {
  
  theme.addEventListener( usingClick, addThemes )
  return theme;
}

const themes = document.querySelectorAll( `.theme` )

const themesWithClick = map( themes, addingClickToThem )

const addClickToButton = ( button ) => console.log(`button`, button)

// themes.map( addClickToButton )

// theme.classList.add( `dark-theme` )
console.log(`themesWithClick: `, themesWithClick)
